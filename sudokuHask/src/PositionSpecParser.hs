...
parsePosition :: Text -> Either String (Position (Maybe Char))
...
stringsToPosition :: Int -> [String] -> (Position (Maybe Char))
stringsToPosition n ll = Position charSet $ (listArray ((0,0), (n-1,n-1))) . map toCell $ concat ll where
  toCell '.' = Nothing
  toCell c = Just c
  charSet = DS.toList $ DS.fromList $ nub $ filter (\c -> c /= '.') $ concat ll

render :: (Position (Maybe Char)) -> Text
render pos@(Position _ p) = pack $ intercalate "\n" $ rows where
  rows = map getRow [0..size-1]
  getRow row = intersperse ' ' $ (cellChar row) <$> [0..size-1]
  cellChar row column = toChar $ getCell pos row column
  toChar Nothing = '.'
  toChar (Just c) = c
  size = dim p
